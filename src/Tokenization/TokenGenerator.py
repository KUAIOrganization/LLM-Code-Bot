...
class TokenGenerator:
    ...
    def __init__(self, folder: str, tokenAmount: int, tokensPerRound: int, removesBeforeClean = 10, allowWhitespace = True, allowTextAndSpceial = True):
        self.tokenAmount: int = tokenAmount
        self.heap: MaxFibonocciHeap = MaxFibonocciHeap()
        self.countArray: list[list[FibonacciNode]] = []
        self.tokens: TernarySearchTree = TernarySearchTree()
        self.tokensList: list[str] = []
        self.tokensExist: list[bool] = []
    ...
    @staticmethod
    def getNextTokenGlobal(file, tree: TernarySearchTree) -> int:
        built = ""
        char = file.read(1)
        if not char:
            return -1
