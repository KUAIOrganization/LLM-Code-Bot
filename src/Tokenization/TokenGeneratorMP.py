...
class TokenGeneratorDriverMP(TokenGenerator):
    ...
    def run(self):
        while self.tokenCount() < self.tokenAmount:
            ...
            for i in range(self._threads - 1):
                processes.append(Process(target=TokenGeneratorDriverMP.processFiles, args=(self.fileQueue, self.tokenPairQueue, self.tokens)))
                processes[-1].start()
    ...
    @staticmethod
    def processFiles(fileQueue: JoinableQueue, tokenOutQueue: Queue, tree: TernarySearchTree):
        while True:
            file = None
