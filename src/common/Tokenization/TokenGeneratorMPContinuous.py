...
class TokenGeneratorDriverMP(TokenGenerator):
    def __init__(self, directory: str, tokenAmount: int, tokensPerRound: int, threads:int = 0, removesBeforeClean = 10, allowWhitespace = True, allowTextAndSpecial = True, startingTokens = None):
        super().__init__(directory, tokenAmount, tokensPerRound, removesBeforeClean, allowWhitespace, allowTextAndSpceial=allowTextAndSpecial, startingTokens=startingTokens)
    ...
    def run(self):
        ...
        for i in range(self._threads - 1):
                ...
                processes.append(Process(target=TokenGeneratorDriverMP.processFiles, args=(self.fileQueue, self.tokenPairQueue, self.tokens, self.outQueues[i], self._allowWhitespace)))
                processes[i].start()
    ...
    @staticmethod
    def processFiles(fileQueue: JoinableQueue, tokenOutQueue: Queue, tree: TernarySearchTree, newTokenQueue: Queue, allowWhitespace: bool):
        while True:
            file = None
            startedFile = False
    ...
    def updateTree(tree: TernarySearchTree, queue: Queue):
        while not queue.empty():
            newPair = queue.get()
            tree.add(newPair[0], newPair[3])
            if len(newPair[1]) > 1:
                tree.remove(newPair[1])
            if len(newPair[2]) > 1:
                tree.remove(newPair[2])
    ...
    def addBaseToken(self, token: str, base0: str, base1: str):
        ...
        for queue in self.outQueues:
            queue.put([token, base0, base1, len(self.tokensList)])
